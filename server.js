...
function linesFrom(room,cell,player){
 ...
 for(const [dx,dy] of dirs) for(let offset=-4;offset<=0;offset++){
   const pts=[]; for(let n=0;n<5;n++){let x=cell.x+(offset+n)*dx,y=cell.y+(offset+n)*dy;if(x<0||y<0||x>11||y>11){pts.length=0;break}pts.push(room.board[y*12+x]);}
